...
class Topcam(fme.Camera):
    ...
    def get_head_body_yaw(self):
        ...
        smooth_x = fme.convfilt(fme.nanmedfilt(self.xrpts.sel(                              \
                                point_loc='center_neck_x').values, 7).squeeze(), box_pts=20)
        
        smooth_y = fme.convfilt(fme.nanmedfilt(self.xrpts.sel(                              \
    ...
    def save_params(self):
        ...
        savepath = os.path.join(self.recording_path,
                                str(self.recording_name+'_'+self.camname+'.nc'))
