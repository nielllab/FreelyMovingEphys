"""
fmEphys/utils/prelimRF_raw.py

Map receptive fields without KS/Phy2 spike sorting. Instead,
the responses will be mapped for each site of the ephys probe
by filtering the LFP to detect spikes.
...
def read_ephys_bin(binary_path, probe_name, do_remap=True,
                   mapping_json=None):
    """ Read ephys binary.
    ...
    path within the utils directory ('probes.json').
...
def open_time(path, dlc_len=None, force_shift=False):
    """ Read in the timestamps for a camera and adjust to deinterlaced video
    length if needed
    Parameters:
    path (str): path to a timestamp .csv file
    dlc_len (int): number of frames in the DLC data (used to decide if interpolation
    is needed, but this can be left as None to ignore)
    ...
    # in case header == 0, which is true of some files, drop that header
    # which will have been read in as the first entry 
    if read_time[0] == 0:  
    ...
    for c in range(n_units):
        ...
        if pd.isna(sta) is True:
            ...
            plt.imshow((sta-np.mean(sta)),
                       vmin=-starange, vmax=starange, cmap='jet')
            staAll[c,:,:] = sta
...
def prelimRF_raw(whitenoise_directory, probe):
    ...
    filt_ephys = butter_bandpass(ephys_center_sub,
                                 lowcut=800, highcut=8000, fs=30000, order=6)
    ...
    world_vid = np.empty([f_count,
                        ...
                        int(vidread.get(cv2.CAP_PROP_FRAME_WIDTH)*0.25)],
                        dtype=np.uint8)
    ...
    plt.subplots(np.size(filt_ephys,1), 1,
                 figsize=(5,int(np.ceil(np.size(filt_ephys,1)/2))))
    ...
    all_STA, fig = plot_prelim_STA(all_spikeT, img_norm, worldT, movInterp,
                                   np.size(filt_ephys,1))
