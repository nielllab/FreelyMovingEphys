...
for trial_path in trial_paths:
    ...
    if 'TOP' in config['camera_names']:
        ...
        plot_topdown_vid(top_avi, pts, head_theta, config, t_name)
    ...
    for i in range(0,len(eye_sides)):
        ...
        # filter the list of files for the current trial to get the topdown view
        eye_h5 = list(filter(lambda a: (eye_side+'EYE') in a, trial_cam_h5))
        eye_csv = list(filter(lambda a: (eye_side+'EYE') in a, trial_cam_csv))
        eye_avi = list(filter(lambda a: (eye_side+'EYE') in a, trial_cam_avi))
        # make an xarray of dlc point values out of the found .h5 files
        # also assign timestamps as coordinates of the xarray
        ...
        print('finding pupil rotation (slow)')
    ...
    # for num in range(0,len(otherhs)):
    #     current_otherh5 = otherh5s[otherh5s]
    #
    #     otherdlc, othernames = h5_to_xr(otherh5, othercsv)
    ...
    #     sideout = xr.concat([sideout, gatheredside], dim='trial', fill_value=np.nan)
        ...
        # TO DO: if there's a camera name that's not recognized, and .h5 files are present, read it in and save points out as an xarray
