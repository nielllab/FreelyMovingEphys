...
def quick_revchecker_analysis(rc_path, probe_type):
    ...
    deinterlace_data(temp_config['data_path'], world_vids, world_times)
    # run calibration
    ...
    recording_names = [i for i in list_subdirs(temp_config['data_path']) if 'hf' in i or 'fm' in i]
    recording_paths = [os.path.join(temp_config['data_path'], recording_name) for recording_name in recording_names]
    ...
    for dir_name in recordings_dict:
        print('reading worldcam data')
        temp_config['recording_path'] = recordings_dict[dir_name]
        recording_name = '_'.join(os.path.splitext(os.path.split([i for i in find('*.avi', temp_config['recording_path']) if all(bad not in i for bad in ['plot','IR','rep11','betafpv','side_gaze'])][0])[1])[0].split('_')[:-1])
        print('opening pdf')
        pdf = PdfPages(os.path.join(dir_name, (recording_name + '_prelim_revchecker_figures.pdf')))
        trial_cam_csv = find(('*BonsaiTS*.csv'), temp_config['recording_path'])
        trial_cam_avi = find(('*.avi'), temp_config['recording_path'])
        ...
        if temp_config['save_nc_vids'] is True:
            ...
            trial_world_data.to_netcdf(os.path.join(temp_config['recording_path'], str(recording_name+'_world.nc')), engine='netcdf4', encoding={'WORLD_video':{"zlib": True, "complevel": 4}})
        elif temp_config['save_nc_vids'] is False:
            worlddlc.to_netcdf(os.path.join(temp_config['recording_path'], str(recording_name+'_world.nc')))
        ...
        ephys_binary_file = os.path.join(dir_name, recording_name + '_ephys_merge.json')
