...
class RunningBall(BaseInput):
    def __init__(self, config):
        ...
        # output sample rate (data will be set up to match this sample rate, since there is not constant sample rate for optical mouse data from Bonsai)
        self.ball_samprate = 0.008
        # float in seconds, window in which a previous timestamp in sparse_time must fall, otherwise a zero will be filled in
