...
from utils.aux_funcs import nanxcorr, find, list_subdirs
...
class Ephys(BaseInput):
    ...
    def sta(self, lag=2):
        ...
        plt.subplots(np.ceil(self.n_units/7).astype('int'), 7, figsize=(35,np.int(np.ceil(self.n_units/3))), dpi=50)
        for c, ind in enumerate(self.cells.index):
            ...
            nsp = np.sum(sp)
            plt.subplot(np.ceil(self.n_units/7).astype('int'), 7, c+1)
    ...
    def tuning_to_variable(self, variable, variable_range, useT, label):
        scatter = np.zeros((self.n_units, len(variable)))
        tuning = np.zeros((self.n_units, len(variable_range)-1))
        ...
        fig = plt.subplots(np.ceil(self.n_units/7).astype('int'), 7, figsize=(35,np.int(np.ceil(self.n_units/3))), dpi=50)
        for i, ind in enumerate(self.cells.index):
            plt.subplot(np.ceil(self.n_units/7).astype('int'), 7, i+1)
    ...
    def saccade_psth(self, right, left, label):
        rightavg = np.zeros((self.n_units, self.trange.size-1))
        leftavg = np.zeros((self.n_units, self.trange.size-1))
        fig = plt.subplots(np.ceil(self.n_units/7).astype('int'), 7, figsize=(35,np.int(np.ceil(self.n_units/3))), dpi=50)
        for i, ind in enumerate(self.cells.index):
    ...
    def fit_glm_rfs(self, nks):
        ...
        # image dimensions
        nk  = nks[0] * nks[1]
        n_units = np.shape(self.model_nsp)[0]
    ...
    def worldcam_at_new_timebase(self, dwnsmpl=0.5):
        ...
        for i, ind in enumerate(self.cells.index):
            self.model_nsp[i,:], bins = np.histogram(self.cells.at[ind,'spikeT'], bins)
