...
class HeadFixedGratings(Ephys):
    ...
    def gratings_analysis(self, xrg=40, yrg=25):
        ...
        plt.figure()
        plt.plot(ori_cat[0:10]); plt.title('ori cat')
        ...
        for c, ind in enumerate(self.cells.index):
            ...
            plt.subplot(self.n_units, 4, 4*c+3)
            ...
            plt.subplot(self.n_units, 4, 4*c+4)
    ...
    def overview_fig(self):
        ...
        for i, ind in enumerate(self.cells.index):
            ...
            # plot tuning curve
            plt.subplot(self.n_units, 4, i*4+3)
            plt.plot(self.ori_x, self.ori_tuning[i,:,0], label='low sf')
            plt.plot(self.ori_x, self.ori_tuning[i,:,1], label='mid sf')
            plt.plot(self.ori_x, self.ori_tuning[i,:,2], label='high sf')
            plt.plot([np.min(self.ori_x), np.max(self.ori_x)], [self.drift_spont[i], self.drift_spont[i]], 'r:', label='spont')
            try:
                plt.ylim(0, np.nanmax(self.ori_tuning[i,:,:]*1.2))
            except ValueError:
                plt.ylim(0,1)
    ...
    def save(self):
        ...
        for unit_num, ind in enumerate(self.cells.index):
            ...
            unit_df = pd.DataFrame(pd.Series([self.contrast_range,
                                    ...
                                    self.grating_ori[unit_num],
                                    self.ori_tuning[unit_num],
