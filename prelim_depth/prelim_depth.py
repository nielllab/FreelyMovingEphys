...
def main(binary_path, probe_type):
    print('opening pdf')
    recording_directory = os.path.split(binary_path)[0]
    pdf = PdfPages(os.path.join(recording_directory, 'prelim_depth.pdf'))
    print('starting continuous LFP laminar depth estimation')
    print('loading ephys binary file and applying filter')
    # read in ephys binary
    lfp_ephys = read_ephys_bin(binary_path, probe_type, do_remap=True)
    ...
    if ch_num == 64:
        ...
        plt.ylim([32,-1]); plt.yticks(ticks=range(32,-1),labels=(ch_spacing*np.arange(32)-layer5_cent_sh0))
        plt.title('shank0')
        plt.subplot(1,2,2)
        plt.plot(norm_profile_sh1,range(0,32))
        plt.plot(norm_profile_sh1[layer5_cent_sh1]+0.01,layer5_cent_sh1,'r*',markersize=12)
        plt.ylim([32,-1]); plt.yticks(ticks=range(32,-1),labels=(ch_spacing*np.arange(32)-layer5_cent_sh1))
        plt.title('shank1')
        ...
        plt.plot(norm_profile_sh0[layer5_cent_sh0]+0.01,layer5_cent_sh0,'r*',markersize=12)
        plt.ylim([16,-1]); plt.yticks(ticks=range(16,-1),labels=(ch_spacing*np.arange(16)-layer5_cent_sh0))
        plt.title('shank0')
        pdf.savefig(); plt.close()
    elif ch_num == 128:
        norm_profile_sh0 = lfp_power_profiles_filt[:32]/np.max(lfp_power_profiles_filt[:32])
        ...
        plt.ylim([32,-1]); plt.yticks(ticks=range(32,-1),labels=(ch_spacing*np.arange(32)-layer5_cent_sh2))
        plt.title('shank2')
        plt.subplot(1,4,4)
        plt.plot(norm_profile_sh3,range(0,32))
        plt.plot(norm_profile_sh3[layer5_cent_sh3]+0.01,layer5_cent_sh3,'r*',markersize=12)
        plt.ylim([32,-1]); plt.yticks(ticks=range(32,-1),labels=(ch_spacing*np.arange(32)-layer5_cent_sh3))
        plt.title('shank3')
        pdf.savefig(); plt.close()