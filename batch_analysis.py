...
"""
import argparse, traceback, yaml, os, platform
import pandas as pd
...
def get_args():
    ...
    parser.add_argument('--csv_filepath', type=str, help='read path for metadata .csv', default='T:/OptoPreyCapture/csv_today.csv')
...
def main(csv_filepath, config_path, log_dir, clear_dlc):
    ...
    if platform.system() == 'Linux':
        for ind, row in csv.iterrows():
            ...
            csv[ind,'animal_dirpath'] = os.path.expanduser('~/'+('/'.join([row['computer'].title(), drive] + list(filter(None, row['animal_dirpath'].replace('\\','/').split('/')))[2:])))
        ...
        if row['run_preprocessing'] == any(['TRUE', True]) or row['run_ephys_analysis'] == any(['TRUE', True]):
            ...
            try:
                if (row['run_ephys_analysis']==True) & (row['run_preprocessing']==True):
                    ...
                    analyze_session(config, clear_dlc=clear_dlc, force_probe_name=None, force_flip_gx_gy=False, batch=True)
